...
import { readNativeProfileOverlay } from "../src/profile-overlay.mjs";
...
await main({
  ...
  serve: async (state) => {
    ...
    const launch = JSON.parse(
      await readFile(join(state, "launch.json"), "utf8"),
    );
    requireThat(
      /^[a-zA-Z0-9_-]{1,40}$/.test(launch.profile),
      "PROFILE_INVALID",
    );
    const bin = await executable();
    process.env.REMOTEDESK_DSH_STATE = state;
    requireThat(
      launch.ownedProfile === true || launch.profile === "web",
      "DSH_PROFILE_NOT_OWNED",
    );
    requireThat(
      launch.profilePath ===
        resolve(
    ...
    const installedRoot = dirname(
      dirname(profileRequire.resolve("@remotedesk/dsh-plugin")),
    );
    ...
    requireThat(
      installedPackage.name === launcherPackage.name &&
    ...
    const patch = await readNativeProfileOverlay(
      ...
      join(installedRoot, "presets"),
      launch.ownedProfile,
    );
    const overlay = join(state, "native-profile.patch.json");
    await writeFile(overlay, JSON.stringify(patch), { mode: 0o600 });
    process.argv.push("--patch", overlay);
    if (launch.webPort !== undefined) {
      requireThat(
        Number.isInteger(launch.webPort) &&
          launch.webPort > 0 &&
          launch.webPort < 65536,
        "WEB_PORT_INVALID",
      );
      process.argv.push(
        "--host",
  ...
  extra: async (command, state, config, o) => {
    ...
    const webPort =
      o["web-port"] === undefined ? undefined : Number(o["web-port"]);
    requireThat(
      webPort === undefined ||
        (Number.isInteger(webPort) && webPort > 0 && webPort < 65536),
      "WEB_PORT_INVALID",
    );
    await withLifecycleLock(
      ...
      () =>
        withLifecycleLock(state, async () => {
          ...
          const launch = await prepareProfileInstall(state, {
            profile,
            packageSha256,
            webPort,
          });
          ...
          try {
            ...
            requireThat(
              (await lstat(localArchive)).isFile(),
              "PROFILE_CACHE_FILE_REQUIRED",
            );
            requireThat(
              createHash("sha256")
          ...
          const nativeSpec =
            "file:remotedesk-packages/" + packageSha256 + ".tgz";
          await new Promise((resolve, reject) => {
            ...
            child.once("exit", (code) =>
              code === 0
                ? resolve()
                : reject(new Error("PLUGIN_INSTALL_FAILED")),
            );
          });
          await writeFile(
    ...
    console.log(
      JSON.stringify({ installed: true, profile, restartRequired: true }),
    );
    return true;
  },
});
