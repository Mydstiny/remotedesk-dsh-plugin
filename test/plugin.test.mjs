...
test("does not observe unselected sessions or retain content", () => {
  ...
  const agent = { id: "one", session };
  h.live.set("one", agent);
  h.sessions.set("one", session);
  ...
  assert.ok(
    !JSON.stringify(h.ctx.remotedeskProbe.status()).includes("SECRET_SENTINEL"),
  );
});
...
test("revokes retained plugin handles on dispose", () => {
  const h = harness();
  const api = h.ctx.remotedeskProbe;
  h.emit("dispose");
...
test("releases the exact disposed agent without evicting a replacement", () => {
  const h = harness();
  const session = { id: "one" };
  const first = { id: "one", session };
  h.live.set("one", first);
  h.sessions.set("one", session);
