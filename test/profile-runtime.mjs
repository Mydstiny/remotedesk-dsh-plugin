// Native package installation and full profile regression. POSIX Docker gate;
// all three OSes separately exercise the real AgentLoop and native service.
...
import { init, addProject, configuration, invite } from '@remotedesk/bridge-core/admin';
...
const scratch = await realpath(await mkdtemp(join(tmpdir(), 'remotedesk-profile-test-')));
const image =
...
try {
  const packed = JSON.parse(
    (
      await command('npm', ['pack', '--json', '--ignore-scripts', '--pack-destination', scratch], {
        cwd: source,
      })
    ).stdout,
  )[0];
  for (const profile of ['remotedesk-test', 'web']) {
    ...
    await mkdir(fixture);
    await writeFile(join(root, 'outside.txt'), 'REMOTEDESK_OUTSIDE_INSTRUCTION_CANARY');
    ...
    await copyFile(join(source, 'test/fixtures/profile-provider.mjs'), join(fixture, 'index.mjs'));
    await copyFile(join(source, 'test/fixtures/profile-local.mjs'), join(fixture, 'local.mjs'));
    await writeFile(
    ...
    const fp = JSON.parse(
      (
        await command('npm', ['pack', '--json', '--ignore-scripts', '--pack-destination', root], {
          cwd: fixture,
          env,
        })
      ).stdout,
    ...
    try {
      ...
      while (!output.includes('"ready":true') && Date.now() < until && child.exitCode === null)
        await delay(100);
      ...
      for (let i = 0; i < 2; i++) {
        assert.equal(
          (await call('turn.start', { sessionId: id, lease, text: 'Reply with test marker.' }))
            .status,
          'succeeded',
        );
        let snapshot;
        ...
        while (Date.now() < end) {
          snapshot = (await client.read('session.read', { sessionId: id })).snapshot;
          if (
      ...
      for (const row of rows.filter((r) => r.remote)) {
        ...
        assert.deepEqual(row.tools.sort(), [
          'remotedesk_question',
          'remotedesk_workspace_exec',
          'remotedesk_workspace_read',
        ]);
      }
      if (profile === 'web') {
        assert.ok(
        ...
        assert.equal(JSON.parse(await readFile(join(root, 'local-result.json'))).status, 'idle');
      }
      console.log(
        'PASS native packed DSH profile ' +
          profile +
          ': mTLS sessions, model turns, archive/resume, exact remote tools, no outside AGENTS read' +
