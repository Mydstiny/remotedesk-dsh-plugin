...
const root = await mkdtemp(join(tmpdir(), 'remotedesk-dsh-loop-'));
const workspace = join(root, 'workspace');
await mkdir(workspace);
const runtime = process.argv[2] ?? (await locateRuntime());
...
class Fixture extends LlmAdapter {
  async *stream(options) {
    ...
    if (mode !== 'text') {
      ...
      const block = {
        type: 'tool-call',
        id: 'fixture-call-' + schemas.length,
        name,
    ...
    yield { type: 'text-delta', index: 0, text: 'DSH_FIXTURE_OK' };
    yield { type: 'block-end', index: 0, block: { type: 'text', text: 'DSH_FIXTURE_OK' } };
...
try {
  for (const [name, config] of [
    ...
    const module = await load('@deepseek-ai/' + name);
    await ctx.plugin(module.default, config);
  }
  ctx.llm.registerAdapter(['fixture'], new Fixture());
  const originalQuestions = ctx.on('user-questions/request', async () => {
    questionCalls++;
    return { answers: [{ id: 'unchanged', selected: [], custom: 'existing UI' }] };
  });
  ...
  const s = { id: 'remote-fixture', project: 'p' };
  await adapter.create(s);
  const turn = async (mode) => {
    selected = mode;
    await adapter.start(s, 'Fixture.');
    const a = ctx.agents.get(s.id);
    await a.whenIdle();
  ...
  assert.ok(schemas.every((s) => !s.includes('host_danger') && !s.includes('run_code')));
  approved = false;
  await turn('exec');
  assert.equal(toolCalls, 1);
  await turn('late_host_tool');
  assert.equal(hostCalls, 0);
  assert.ok(events.some((e) => JSON.stringify(e).includes('REMOTEDESK_TOOL_SCOPE_DENIED')));
  await turn('run_code');
  assert.equal(hostCalls, 0);
  await turn('question');
  assert.equal(questionCalls, 0);
  assert.ok(events.some((e) => JSON.stringify(e).includes('remote answer')));
  await ctx.userQuestions.ask({
    questions: [{ id: 'unchanged', question: 'Existing UI?', options: [] }],
  });
  assert.equal(questionCalls, 1);
  selected = 'hang';
  await adapter.start(s, 'Cancel this.');
  await new Promise((r) => setTimeout(r, 50));
  await adapter.cancel(s);
  assert.equal(ctx.agents.get(s.id).status, 'idle');
  const before = await adapter.read(s);
  ...
  originalQuestions();
  console.log(
    'PASS real DSH AgentLoop: model and tools, approve/deny, scoped late-tool/PTC rejection, existing questions UI, cancel, persistence, dispose/resume. Executor is a deterministic fixture; Docker has a separate test.',
  );
} finally {
  await adapter?.close();
