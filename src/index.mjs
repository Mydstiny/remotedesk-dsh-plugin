...
export function apply(ctx) {
  ...
  const isLive = agent => !disposed && ctx.agents.get(agent.id) === agent && ctx.sessions.get(agent.id) === agent.session;
  const assertActive = () => { if (disposed) throw new Error('PROBE_DISPOSED'); };
  const api = Object.freeze({
    observe(agent) {
      assertActive();
      if (!agent || !isLive(agent)) throw new Error('AGENT_NOT_EXACT_LIVE_INSTANCE');
      selected.set(agent.id, agent);
      return () => { if (selected.get(agent.id) === agent) selected.delete(agent.id); };
    },
    status() {
      assertActive();
      return { stage: 'AI0', remoteAccess: false, acceptedEvents, observedAgents: selected.size,
        userQuestions: 'untouched', contentRetained: false };
    }
  });
