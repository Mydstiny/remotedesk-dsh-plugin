...
export async function requirePnpm() {
  for (const dir of (process.env.PATH ?? "").split(delimiter).filter(Boolean)) {
    try {
      await access(
        join(dir, process.platform === "win32" ? "pnpm.cmd" : "pnpm"),
        constants.X_OK,
      );
      return;
    } catch {}
...
export async function prepareProfileInstall(
  ...
  const [old, intent] = await Promise.all([
    read(join(state, "launch.json")),
    read(intentPath),
  ]);
  let ownedProfile = false;
  try {
    requireThat(
      (await lstat(profilePath)).isDirectory(),
      "DSH_PROFILE_DIRECTORY_REQUIRED",
    );
