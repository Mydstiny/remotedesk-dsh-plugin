  ...
  for (const directory of (process.env.PATH ?? "")
    ...
    try {
      ...
      if (process.platform === "win32") {
        ...
        try {
          if (
            (await metadata(join(root, "package.json"))).name ===
            "@deepseek-ai/dsh"
          )
            return root;
        } catch {}
      ...
      for (let i = 0; i < 4; i++) {
        try {
          if (
            (await metadata(join(candidate, "package.json"))).name ===
            "@deepseek-ai/dsh"
          )
            return candidate;
        } catch {
...
export async function doctor({ runtimeRoot } = {}) {
  const report = {
    ...
    changed: false,
    componentVersions: {
      adapter: compatibility.adapterVersion,
    ...
    requiresUserAction: [],
    warnings: [
      "NATIVE_SANDBOX_AND_APPROVALS",
      "PROVIDER_AUTHENTICATION_STAYS_ON_HOST",
    ],
    capabilities: {
      remoteAccess: false,
      remoteProtocol: 1,
      proEntitlement: "pro.lifetime",
    },
  };
  try {
    if (Number(process.versions.node.split(".")[0]) < 22)
      throw new Error("NODE_VERSION_UNSUPPORTED");
    const versions = await inspectRuntime(
      runtimeRoot ?? (await locateRuntime()),
    );
    Object.assign(report.componentVersions, versions);
    for (const [name, version] of Object.entries(versions)) {
      const expected =
        name === "@deepseek-ai/dsh"
          ? compatibility.cliVersions.includes(version)
          : compatibility.components[name] === version;
      report.checks.push({
        id: name,
        status: expected ? "pass" : "fail",
        code: expected ? "PINNED_VERSION" : "UNVERIFIED_COMPONENT_VERSION",
      });
    }
    report.status = report.checks.every((c) => c.status === "pass")
      ? "ok"
      : "blocked";
  } catch (error) {
    const safeCodes = [
      "DSH_RUNTIME_NOT_FOUND_USE_RUNTIME_ROOT",
      "NOT_DSH_INSTALLATION",
    ...
    report.checks.push({
      id: "runtime",
      status: "fail",
      code: safeCodes.includes(error.message)
        ? error.message
        : "RUNTIME_METADATA_UNAVAILABLE",
    });
  }
  if (report.status !== "ok")
    report.requiresUserAction.push("CHECK_EXACT_COMPONENT_VERSIONS");
  return report;
}
