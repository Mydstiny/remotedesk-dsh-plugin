...
import {
  ...
  projectDiff,
} from '@remotedesk/bridge-core/workspace-tools';
import { DockerExecutor } from './docker-executor.mjs';
const PUBLIC_EVENT =
  /^(user\/message|assistant\/(chunk|message)|tool\/(call|result)|turn\/(start|end)|step\/(start|end))$/;
const output = {
...
export class DshAdapter {
  capabilities = {
    ...
    execution: 'docker-project-mount-no-network',
  ...
  bind(core) {
    ...
    this.disposers.push(
      this.ctx.on('session/event', (session, event) => {
        ...
        if (!h || h.agent.session !== session || this.ctx.agents.get(session.id) !== h.agent)
          return;
        if (PUBLIC_EVENT.test(event.type))
          core.emit(session.id, { type: event.type, seq: event.seq, data: event.data });
      }),
    );
  }
  async prepare() {
    ...
    for (const p of this.core.projects) await this.executor.check(p);
  }
  project(s) {
  ...
  options(p, s = {}) {
    ...
    const provider = s.provider ?? p.provider ?? defaults.provider,
      model = s.model ?? p.model ?? defaults.model;
    const reasoningEffort =
      s.reasoningEffort ??
      (!s.provider && !s.model && provider === defaults.provider && model === defaults.model
  ...
  setup(s, p) {
    return (agentCtx) => {
      agentCtx.on('agent/pre-step', (_event, next) => {
        assertProfile(this.ctx);
        return next();
      });
      ...
      agentCtx.tools.guard((exec) => {
        const handle = this.handles.get(s.id),
          definition = agentCtx.tools.get(exec.name, exec.agent);
        return this.closed ||
          !ready ||
          !handle ||
          handle.agent !== exec.agent ||
          this.ctx.agents.get(s.id) !== exec.agent ||
          exec.parent ||
          !expected.has(exec.name) ||
          definition?.execute !== expected.get(exec.name)
          ? 'REMOTEDESK_TOOL_SCOPE_DENIED'
          : undefined;
      });
      const register = (definition) => {
        expected.set(definition.name, definition.execute);
  ...
  async connect(s, create = false) {
    ...
    if (old) {
      requireThat(this.ctx.agents.get(s.id) === old.agent, 'AGENT_INSTANCE_CHANGED');
      return old;
    }
    if (this.loading.has(s.id)) return this.loading.get(s.id);
    const p = this.project(s);
    const pending = (async () => {
      await this.executor.check(p);
      const selection = this.options(p, s);
      requireThat(
      ...
      const opts = { agentOptions: selection, setup: this.setup(s, p) };
      const handle = await (create
        ? this.ctx.agents.create({ ...opts, sessionId: s.id, meta: { cwd: p.path } })
        : this.ctx.agents.resume({ ...opts, resumeSessionId: s.id }));
      if (this.closed) {
  ...
  async read(s, { cursor } = {}) {
    const { agent } = await this.connect(s);
    const from = cursor === undefined ? 0 : Number(cursor);
    requireThat(Number.isSafeInteger(from) && from >= 0, 'CURSOR_INVALID');
    ...
    return {
      ...
      inputModalities: s.inputModalities,
      events,
      nextCursor: rows.length > events.length ? String(events.at(-1).seq + 1) : null,
    };
  }
  async start(s, text, attachments = []) {
    ...
    try {
      const { agent } = await this.connect(s);
      requireThat(agent.status === 'idle', 'TURN_ALREADY_RUNNING');
      const images = attachments.filter((a) => a.mime !== 'text/plain');
      let refs = [];
      if (images.length) {
        requireThat(s.inputModalities?.includes('image'), 'MODEL_IMAGE_CAPABILITY_UNDECLARED');
        requireThat(this.ctx.attachments, 'IMAGE_STORE_UNAVAILABLE');
        refs = await this.ctx.attachments.saveImages(
          images.map((a) => ({ data: Buffer.from(a.data, 'base64'), mediaType: a.mime })),
        );
      }
      ...
      const content = [
        ...
        ...attachments.map((a) =>
          a.mime === 'text/plain'
            ? { type: 'text', text: Buffer.from(a.data, 'base64').toString('utf8') }
            : { type: 'image', attachment: refs[i++] },
        ),
      ];
      agent.followup({ id: run.id, role: 'user', source: { kind: 'user' }, content });
      run.agent = agent;
      run.readyResolve();
      ...
      if (this.runs.get(s.id) === run && !run.agent) this.runs.delete(s.id);
      throw e;
    } finally {
      run.readyResolve();
    }
  ...
  async steer(s, text) {
    ...
    requireThat(run?.agent && !run.cancelled && run.agent.status === 'running', 'NO_ACTIVE_TURN');
    const id = randomUUID();
    run.agent.steer({
      id,
      role: 'user',
  ...
  async cancel(s) {
    ...
    const agent = this.handles.get(s.id)?.agent;
    if (agent) {
  ...
  async close() {
    ...
    this.closed = true;
    for (const r of this.runs.values()) r.cancelled = true;
