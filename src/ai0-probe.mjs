...
export function apply(ctx) {
  ...
  const isLive = (agent) =>
    !disposed && ctx.agents.get(agent.id) === agent && ctx.sessions.get(agent.id) === agent.session;
  const assertActive = () => {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (agent && isLive(agent) && agent.session === session && Number.isSafeInteger(event.seq))
      acceptedEvents++;
  });
